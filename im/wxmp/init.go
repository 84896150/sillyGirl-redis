...
func init() {
	...
	if !wxmp.GetBool("isKe?", false) {
		...
		core.Server.Any("/wx/receive1", func(c *gin.Context) {
			key := c.Query("key")
			logs.Info("开始监听,", `c.Request=`, c.Request, "c.Writer=", c.Writer, "key=", key)
			sender := &Sender{}
			sender.Message = key
			sender.Wait = make(chan []interface{}, 1)
			sender.uid = fmt.Sprint("111")
			sender.tp = "wxmp"
			core.Senders <- sender
			end := <-sender.Wait
			logs.Info("开始监听,", `end=`, end, "sender.Message=", sender.Message)
			ss := []string{}
			if len(end) == 0 {
				ss = append(ss, wxmp.GetString("default_reply", "无法回复该消息"))
			}
			...
			logs.Info("开始监听,", `end=`, end, "ss=", ss)
			c.JSON(200, map[string]string{"code": "1", "data": ss[0]})
		})
